...
from molviz.core import Candidate, read
from molviz.core.export import ExportItem, to_colab, to_marimo
...
def test_build_dsc_figure_overlays_segments(dsc_txt):
    from molviz.app.plotting import build_dsc_figure
...
def test_export_marimo_is_valid_python():
    ...
    compile(src, "molviz_export.py", "exec")  # raises SyntaxError if malformed
...
def test_export_colab_is_valid_notebook():
    ...
    assert any("molviz" in c.source for c in nb.cells)


def test_app_factory_registers_three_pages():
    import dash

    from molviz.app.server import create_app
