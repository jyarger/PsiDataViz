...
from molviz.app.plotting import build_figure
from molviz.app.services import load_dataset
...
def export(_m_clicks, _c_clicks, selection, x_quantity):
    ...
    if triggered == "dl-marimo":
        return dcc.send_string(to_marimo(items, x_quantity=x_quantity), "molviz_export.py")
    if triggered == "dl-colab":
        data = to_colab(items, x_quantity=x_quantity)
        return dcc.send_bytes(lambda buffer: buffer.write(data), "molviz_export.ipynb")
    return no_update
