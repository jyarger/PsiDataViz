...
No database — just content-addressed files under the user cache dir. Set ``MOLVIZ_NO_CACHE=1`` to
...
CACHE_DIR = Path(os.environ.get("MOLVIZ_CACHE_DIR", platformdirs.user_cache_dir("molviz")))


def _enabled() -> bool:
    return os.environ.get("MOLVIZ_NO_CACHE", "") not in ("1", "true", "True")
