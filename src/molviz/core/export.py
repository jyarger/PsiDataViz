...
Both artifacts are self-contained: they ``pip install`` molviz, fetch the exact source files by
...
#: Package spec the generated notebooks install. Override once molviz is published to PyPI or
#: pin to a git URL, e.g. "molviz @ git+https://github.com/yargerlab/molviz".
PACKAGE_SPEC = "molviz"
...
def to_colab(items: list[ExportItem], x_quantity: str = "temperature") -> bytes:
    ...
    nb.cells = [
        nbformat.v4.new_markdown_cell(
            "# MolViz export\n\n"
            f"Interactive view of {len(items)} dataset(s), generated by MolViz. "
